def dragon_solution(is_dead, number_of_cows):
    ...
    while (fat_alive_cow_index) <= thin_alive_cow_index: # bináris keresés
        ...
        if is_dead(middle_cow): # megnézzük, hogy a középső tehén halott-e
            fat_alive_cow_index = middle_cow + 1 # ha igen, akkor a ducibb tehenek oldalán folytatjuk a keresést
        else:
            thin_alive_cow_index = middle_cow - 1 # ha nem, akkor a vékonyabb tehenek oldalán folytatjuk a keresést
    
    return fat_alive_cow_index # visszaadja a legducibb élő tehenet 